...
import { Calendar, Clock, MapPin, Plus, Trash2, X, Plane, Hotel, Navigation, Loader2, Pencil, Users, ShieldCheck } from \"lucide-react\";
...
export const ItineraryView: React.FC<ItineraryViewProps> = ({ groupId, currentUserId, userIdentityDoc, initialData }) => {
  ...
  const fetchAll = async () => {
    try {
      const res = await apiFetch(/api/groups//details);
      if (res.ok) {
  ...
  const handleAddDay = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(/api/groups//itinerary, {
        method: \"POST\",
  ...
  const handleDeleteDay = async (day: GroupedDay) => {
    ...
    try {
      await apiFetch(/api/groups//itinerary/day?date=, { method: \"DELETE\" });
  ...
  const handleDeleteActivity = async (itemId: string) => {
    try {
      await apiFetch(/api/itinerary/, { method: \"DELETE\" });
      setDays((prev) =>
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case \"food\": return <span className=\"text-xs\">???</span>;
...
with open('src/components/itinerary/ItineraryView.tsx', 'w', encoding='utf-8') as f:
    f.write(clean_code)
